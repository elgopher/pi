...
type BackendInterface interface {
	...
	SetLoop(ch Chan, start int, stop int, loop LoopType, delay float64)

	// SetPlaybackRate schedules the playback rate change to take effect after the specified delay.
	SetPlaybackRate(ch Chan, rate float64, delay float64)
...
func (p panicBackend) SetPlaybackRate(ch Chan, rate float64, delay float64) {
	panic("cannot set playback rate: backend not set. Please call SetPlaybackRate only after starting the game")
}
