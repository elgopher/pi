...
func TestStretch(t *testing.T) {
	...
	t.Run("clipped by screen boundaries", func(t *testing.T) {
		tests := map[string]struct {
			x, y int
		}{
			"-1,0": {x: -1},
		...
		for testName, testCase := range tests {
			t.Run(testName, func(t *testing.T) {
				pi.SetScreenSize(3, 3)
				pi.Cls()
				pi.Palette = pi.DecodePalette(spritePNG)
				canvas := pi.DecodeCanvas(spritePNG)
				sprite := pi.SpriteFrom(canvas, 1, 1, 3, 3)
				// when
				pi.Stretch(sprite, testCase.x, testCase.y, sprite.W, sprite.H)
				// then
				area := sprite.Area
				area.X -= testCase.x
				area.Y -= testCase.y
				expected := canvas.CloneArea(area)
				pitest.AssertSurfaceEqual(t, expected, pi.Screen())
			})
		}
	})

	t.Run("flipped horizontally", func(t *testing.T) {
		// TODO
	})

	t.Run("flipped vertically", func(t *testing.T) {
		// TODO
	})
